function changeDrpDwn1() {
    
    var checkbox = document.querySelector('.switchPlan input[type="checkbox"]');

    if (checkbox.checked) {
    var titleChangeElement = document.getElementById("freePricing1");
    titleChangeElement.textContent = "5 Pro";


    var priceChangeElement = document.getElementById("free3");
    priceChangeElement.textContent = "£25";

    var drpMenuElement = document.getElementById("dropbtn3");
    drpMenuElement.textContent = "Up to 5 Clients";
    } else  {
    var titleChangeElement = document.getElementById("freePricing1");
    titleChangeElement.textContent = "5 Pro";
    
    var priceChangeElement = document.getElementById("free3");
    priceChangeElement.textContent = "£30";
    
    var drpMenuElement = document.getElementById("dropbtn3");
    drpMenuElement.textContent = "Up to 5 Clients";
    }
}

function changeDrpDwn2() {
    var checkbox = document.querySelector('.switchPlan input[type="checkbox"]');

    if (checkbox.checked) {
        var titleChangeElement = document.getElementById("freePricing1");
        titleChangeElement.textContent = "15 Pro";

        var drpMenuElement = document.getElementById("dropbtn3");
        drpMenuElement.textContent = "Up to 15 Clients";
    } else {
        var titleChangeElement = document.getElementById("freePricing1");
        titleChangeElement.textContent = "15 Pro";

        var priceChangeElement = document.getElementById("free3");
        priceChangeElement.textContent = "£45";

        var drpMenuElement = document.getElementById("dropbtn3");
        drpMenuElement.textContent = "Up to 15 Clients";
        
    }
}



function changeDrpDwn3() {
    var checkbox = document.querySelector('.switchPlan input[type="checkbox"]');

    if (checkbox.checked) {
    var titleChangeElement = document.getElementById("freePricing1");
    titleChangeElement.textContent = "30 Pro";

    var drpMenuElement = document.getElementById("dropbtn3");
    drpMenuElement.textContent = "Up to 30 Clients";
} else {
    var titleChangeElement = document.getElementById("freePricing1");
    titleChangeElement.textContent = "30 Pro";

    var priceChangeElement = document.getElementById("free3");
    priceChangeElement.textContent = "£70";

    var drpMenuElement = document.getElementById("dropbtn3");
    drpMenuElement.textContent = "Up to 30 Clients";
}
}



function changeDrpDwn4() {
    var checkbox = document.querySelector('.switchPlan input[type="checkbox"]');

    if (checkbox.checked) {
    var titleChangeElement = document.getElementById("freePricing1");
    titleChangeElement.textContent = "50 Pro";

    var drpMenuElement = document.getElementById("dropbtn3");
    drpMenuElement.textContent = "Up to 50 Clients";
} else {
    var titleChangeElement = document.getElementById("freePricing1");
    titleChangeElement.textContent = "50 Pro";

    var priceChangeElement = document.getElementById("free3");
    priceChangeElement.textContent = "£120";

    var drpMenuElement = document.getElementById("dropbtn3");
    drpMenuElement.textContent = "Up to 50 Clients";
 
}
}

function changeDrpDwn5() {
    var checkbox = document.querySelector('.switchPlan input[type="checkbox"]');

    if (checkbox.checked) {
    var titleChangeElement = document.getElementById("freePricing1");
    titleChangeElement.textContent = "75 Pro";

    var drpMenuElement = document.getElementById("dropbtn3");
    drpMenuElement.textContent = "Up to 75 Clients";
} else {
    var titleChangeElement = document.getElementById("freePricing1");
    titleChangeElement.textContent = "75 Pro";

    var priceChangeElement = document.getElementById("free3");
    priceChangeElement.textContent = "£165";

    var drpMenuElement = document.getElementById("dropbtn3");
    drpMenuElement.textContent = "Up to 75 Clients";
}
}

function changeDrpDwn6() {
    var checkbox = document.querySelector('.switchPlan input[type="checkbox"]');

    if (checkbox.checked) {
    var titleChangeElement = document.getElementById("freePricing1");
    titleChangeElement.textContent = "100 Pro";

    var drpMenuElement = document.getElementById("dropbtn3");
    drpMenuElement.textContent = "Up to 100 Clients";
} else {
    var titleChangeElement = document.getElementById("freePricing1");
    titleChangeElement.textContent = "100 Pro";

    var priceChangeElement = document.getElementById("free3");
    priceChangeElement.textContent = "£200";

    var drpMenuElement = document.getElementById("dropbtn3");
    drpMenuElement.textContent = "Up to 100 Clients";
}
}

function changeDrpDwn7() {
    var checkbox = document.querySelector('.switchPlan input[type="checkbox"]');

    if (checkbox.checked) {
    var titleChangeElement = document.getElementById("freePricing1");
    titleChangeElement.textContent = "200 Pro";

    var drpMenuElement = document.getElementById("dropbtn3");
    drpMenuElement.textContent = "Up to 200 Clients";
} else {
    var titleChangeElement = document.getElementById("freePricing1");
    titleChangeElement.textContent = "200 Pro";

    var priceChangeElement = document.getElementById("free3");
    priceChangeElement.textContent = "£225";

    var drpMenuElement = document.getElementById("dropbtn3");
    drpMenuElement.textContent = "Up to 200 Clients";  
}
}